...
class Streamlit_YOLOV7(SingleInference_YOLOV7):
    ...
    def predict(self):
        ...
        if len(self.predicted_bboxes_PascalVOC)>0:
            df = pd.DataFrame(columns = ['name','x0','y0','x1','y1','score'])
            for item in self.predicted_bboxes_PascalVOC:
                name=str(item[0])
                conf=str(round(100*item[-1],2))
                df.append({'name':name, 'x0': item[1],'y0': item[2],'x1': item[3],'y1': item[4],'score': conf}, ignore_index=True)
...
if __name__=='__main__': 
    ...
    #INPUTS for YOLOV7
    img_size=1056
    path_yolov7_weights="31_12_2022_05_35_25.PT"
